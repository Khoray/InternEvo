...
class PackedFlashLlama1D(nn.Module):
    ...
    def __init__(
        ...
        if first:
            from internlm.model.llava_modules.projector_builder import build_vision_projector
            self.vision_proj = build_vision_projector(vision_proj_cfg)
            # self.vision_proj.requires_grad_(False)

    def forward(self, hidden_states=None, images=[], cu_seqlens=None, input_ids=None, indexes=None, inference_params=None):
        xs = []
        pure_text = False
        ...
        if hasattr(self, "tok_embeddings"):
            ...
            if pure_text and len(xs) > 0:
                ...
                for i in range(len(xs)):
                    hidden_states[i, org_ids[i] == self.image_token_id] = (xs[i].reshape((-1, xs[i].shape[-1]))).to(hidden_states.dtype)
